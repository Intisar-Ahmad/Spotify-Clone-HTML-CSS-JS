...
function getSongNameFromUrl(href) {
    try {
        let decodedHref = decodeURIComponent(href);
        let normalizedHref = decodedHref.replace(/\\/g, "/"); // Fix Windows backslashes
...
(async function firstfolder() {
    ...
    for (const element of div.querySelectorAll("a")) {
        let href = getFolderNameFromUrl(element.href);
        // console.log(href)
        if (href) {
            currfolder = href;
            main()
...
async function main() {
    ...
    async function displayalbums() {
        let cardcliker = async (item) => {
            deletelist();
            currfolder = getFolderNameFromUrl(item.dataset.folder);
    ...
    displayalbums();
    function makelist() { // done
        for (const song of songs) {
            let li = document.createElement("li");
            let str = getSongNameFromUrl(song);
            // console.log(str)
            str = str.replace(".mp3", "");
            str = str.replaceAll(/%20/g, " ")

            li.innerHTML = `<div><img src = "/images/headphones.svg"><div class = "info">${str}</div></div> <div class = "playnow"><img src = "/images/play.svg"> Play Now </div>`;
            li.innerHTML.trim();
