...
(async function firstfolder(){
    ...
    for (const element of div.querySelectorAll("a")) {
        if (element.href.includes("%5Csongs")) {
            currfolder = (element.href.split("%5C")[2].slice(0,-1))
            main();
            break;
        }
    }
...
async function main() {
    ...
    async function displayalbums() {
        ...
        for (const element of div.querySelectorAll("a")) {
            if (element.href.includes("%5Csongs") && !(element.href.includes(".mp3",".mp4","jpg","png","cpp","py","js"))) {
                anchors.push(element.href.split("%5C")[2].slice(0,-1))
            }
        }
